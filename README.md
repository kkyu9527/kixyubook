...
- 支持跟随系统、日间、夜间和自定义 Hex 阅读配色，并实时展示颜色预览
...
TXT 是可编辑格式。解析器优先使用 Android ICU 自动判断编码，并内置 UTF-8/16/32、GB18030/GBK/GB2312、Big5、Shift-JIS、EUC-JP、EUC-KR、Windows-125x、KOI8-R 等主流编码回退。书名、作者与简介会从正文中剥离；目录解析会将“卷 / 部 / 篇”作为上层分组，将“章 / 节 / 回 / 话”作为实际章节，既能区分第二卷重新从第一章开始，也会保留跨卷连续的原章号。已导入的 TXT 可在书籍管理中选择“重新解析正文”，无需删除后重新导入。书名、作者、简介和正文修改都会保存为独立的用户补丁，不会覆盖原始 TXT 文件。阅读时展示的内容由“原文 + 补丁”组合得到，并为后续撤销能力保留数据基础。
...
├── core-database           Room、导入、补丁、统计、备份与字体
...
图标以两片悬浮页面和中央负空间光束构成“安静阅读空间”，主体约占画布宽度 55%，在圆形、圆角矩形和方形遮罩下都保留充足呼吸感。Android VectorDrawable 由这些 SVG 概念同步维护。
