...
class ReaderPersistenceTest {
    ...
    @Test fun annotationsBookmarksSearchAndProgressSurviveClosingAndReopeningDatabase() = runBlocking(Dispatchers.IO) {
        ...
        try {
            ...
            val annotations = LocalReaderAnnotationRepository(context, db.readerAnnotationDao(), dao, recorder)
            ...
            val repository = LocalReaderAnnotationRepository(context, db.readerAnnotationDao(), reopened, recorder)
    ...
    @Test fun editingSameRangePreservesNoteAndDeletingDoesNotChangeSourceOrSearch() = runBlocking(Dispatchers.IO) {
        ...
        try {
            ...
            val paragraphs = dao.getParagraphs(12)
            val repository = LocalReaderAnnotationRepository(context, db.readerAnnotationDao(), dao, recorder)
