...
// Adapted from Kyant0/AndroidLiquidGlass and compose-miuix-ui/miuix.
...
/** Adds the same rounded-rectangle refraction used by the MIUIX liquid-glass example. */
internal fun BackdropEffectScope.kixyuLens(
    ...
    val key = if (dispersionEnabled) {
        "KixyuLiquidGlassLensDispersion"
    } else {
        "KixyuLiquidGlassLens"
    }
...
private fun BackdropEffectScope.roundedRectCornerRadii(): FloatArray? {
    ...
    val topLeft = if (isLtr) cornerShape.topStart else cornerShape.topEnd
    val topRight = if (isLtr) cornerShape.topEnd else cornerShape.topStart
    val bottomRight = if (isLtr) cornerShape.bottomEnd else cornerShape.bottomStart
    val bottomLeft = if (isLtr) cornerShape.bottomStart else cornerShape.bottomEnd
    return floatArrayOf(
        topLeft.toPx(sizePx, this).fastCoerceAtMost(maxRadius),
        topRight.toPx(sizePx, this).fastCoerceAtMost(maxRadius),
        bottomRight.toPx(sizePx, this).fastCoerceAtMost(maxRadius),
        bottomLeft.toPx(sizePx, this).fastCoerceAtMost(maxRadius),
    )
}
